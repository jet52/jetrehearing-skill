...
### Step 9: Stamp Provenance Footer (final step)

Stamp a provenance footer onto the finished memo so the report carries a reproducibility record — which Claude model and which jetrehearing version generated it, and on what date — for later validation and comparison as the model and the skill change. Run this **last**, after any Step 8 verification append:

```bash
python3 ~/.claude/skills/jetrehearing/scripts/provenance.py --file {memo_file} \
  --model "{runtime model — friendly name and exact ID, e.g. Claude Opus 4.8 (claude-opus-4-8)}"
```

The footer reads, e.g.: *Report generated by Claude Opus 4.8 (claude-opus-4-8) using jetrehearing v1.0.0 on 2026-06-05. AI-generated first draft for internal use; verify all citations and findings before relying.*
...
- **You supply only `--model`**, using the model identifier from your runtime context. The script never infers the model itself, because a model naming its own release is exactly the unreliable self-report a validation record must not depend on.
- The stamp is **idempotent**: re-running replaces the existing footer rather than duplicating it.
